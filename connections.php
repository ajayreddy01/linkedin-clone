...
<body>
      ...
      <div class="layout-page">
        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
              ...
              <div class="col-md-12">
                <div class="nav-align-top">
                  <ul class="nav nav-pills flex-column flex-sm-row mb-6">
                    ...
                    <li class="nav-item">
                      <a class="nav-link active" href="connections.php"><i class="bx bx-link-alt bx-sm me-1_5"></i> Connections</a>
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <h6 class="mb-4">My Connections</h6>
            <div class="row g-6" id="connections-list">
              <!-- Connections will be populated here -->
            </div>

            <!-- Recommended Connections -->
            <h6 class="mt-6 mb-4">Recommended Connections</h6>
  ...
  <script>
    ...
    function loadConnections() {
        fetch('/api/index.php?action=getConnections', {
            method: 'POST',
        ...
        .then(data => {
            console.log('Connections Response:', data); // Debug log
            ...
            if (data.success && data.connections && data.connections.length > 0) {
                data.connections.forEach(conn => {
                    if (conn.status === 'accepted') {
                        const card = createUserCard(conn.connected_user_id, conn.first_name, conn.last_name, conn.profile_picture_url, conn.headline);
                        connectionsList.innerHTML += card;
                    }
                });
            } else {
    ...
    // Fetch and display recommended users
    function loadRecommendedUsers() {
        fetch('/api/index.php?action=recommendUsers', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `user_id=${encodeURIComponent(currentUserId)}&limit=3`
        })
        .then(response => response.json())
        .then(data => {
            console.log('Recommended Users Response:', data); // Debug log
            const recommendedList = document.getElementById('recommended-list');
            recommendedList.innerHTML = '';

            if (data.success && data.users && data.users.length > 0) {
                data.users.forEach(user => {
                    const card = createUserCard(user.user_id, user.first_name, user.last_name, user.profile_picture_url, user.headline, true);
    ...
    function createUserCard(userId, firstName, lastName, avatarUrl, headline, isRecommendation = false) {
        return `
            <div class="col-xl-3 col-lg-4 col-md-6">
                <div class="card">
                    <div class="card-body text-center">
                        <div class="mx-auto my-6">
                            <img src="${avatarUrl || '../../assets/img/avatars/3.png'}" alt="Avatar Image" class="rounded-circle w-px-100 h-px-100" />
                        </div>
                        <h5 class="mb-0 card-title">${firstName || 'Unknown'} ${lastName || ''}</h5>
                        <p class="text-muted">${headline || 'No headline'}</p>
                        ${isRecommendation ? `
    ...
    document.addEventListener('click', function(event) {
        if (event.target.closest('.connect-btn')) {
            const btn = event.target.closest('.connect-btn');
            const connectedUserId = btn.getAttribute('data-user-id');

            fetch('/api/index.php?action=sendConnectionRequest', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: `user_id=${encodeURIComponent(currentUserId)}&connected_user_id=${encodeURIComponent(connectedUserId)}`
            })
            .then(response => response.json())
            .then(data => {
                console.log('Connection Request Response:', data); // Debug log
                if (data.success) {
